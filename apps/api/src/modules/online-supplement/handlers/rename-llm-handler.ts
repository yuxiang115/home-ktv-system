...
- 《》和【】括住的内容通常就是歌名,取括号内的文字作为歌名,不要把 "歌手《歌名》" 整段当歌名。例:"誓言《求佛》" 歌手=誓言,歌名=求佛。
- 标题首段可能是上传者/频道/唱片公司名(如 "YOYOROCK_滾石移動"、含 Records/Official/滾石/移動 等厂牌词),这不是歌手,要从其余部分找真正的歌手;确实找不到时用 "群星"。
